...
const Button = ({ title, icon, handleClick, filled, className, loading }) => {
  return (
    <button
      ...
      } hover:cursor-pointer border px-4 flex items-center  py-2 rounded-md w-full md:w-auto ${className}
      `}
      onClick={handleClick}
    ...
    >
      {loading ? (
                ...
                <p className="flex items-center text-center text-sm font-medium"> <span>{icon && icon}</span> <span>{title}</span></p>
              )}
